...
def run(audio_dir, crepe_model_capacity='full'):
    ...
    for path in wav_files:
        ...
        if file_output_dir.exists():
            ...
            continue
        onset_activations, time, frequency, confidence, rms = features_extractor.get_all_features(str(path))
        save_all_features(file_output_dir, onset_activations, time, frequency, confidence, rms)
...
def main():
    ...
    parser.add_argument('audio_dir', type=Path, help='Path to the directory containing audio files')
    parser.add_argument('--model-capacity', '-c', choices=['tiny', 'full'], default='full', help='Model capacity of CREPE')
